...
namespace disrtibutions {
    ...
    PoissonDistribution::PoissonDistribution(double lambda, double significant_probability) : ProbabilityDistribution()
    {
        ...
        // найдём количество случайных величин для данного significant_probability
    ...
    double PoissonDistribution::getRandomVaraible() {
        ...
        return probs_size_; // если мы пробежались по отрезку и не нашли нужной вероятности, то значит мы в области ма случайных величин, возвращаем probs_size_ - последняя из возможных случ. величин в модели 
    }
    ...
    void PoissonDistribution::test(bool max_sampling) {
        ...
        double score = 0.0;
        for (unsigned int i = 0; i < k; i++) {
            p[i] = pow(exp(1.0f), -lambda_) * pow(lambda_, i) / factorial(i); // нашли теоретическую вероятность
